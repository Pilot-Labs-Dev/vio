...
#include "vio/bundle_adjustment.h"
...
namespace vio {
                ...
                Map* map)
        {
            ...
            std::shared_ptr<g2oFrameSE2> v_frame1 = createG2oFrameSE2(frame1, v_id++, true);
            optimizer.addVertex(v_frame1);

            // New Keyframe Vertex 2
            std::shared_ptr<g2oFrameSE2> v_frame2 = createG2oFrameSE2(frame2, v_id++, false);
            ...
            {
                std::shared_ptr<Point> pt = (*it_ftr)->point;
                if(pt == NULL)
                    continue;
                std::shared_ptr<g2oPoint> v_pt = createG2oPoint(pt->pos_, v_id++, false);
                optimizer.addVertex(v_pt);
                pt->v_pt_ = v_pt;
                std::shared_ptr<g2oEdgeSE3> e = createG2oEdgeSE3(v_frame1, v_pt, vk::project2d((*it_ftr)->f), true, reproj_thresh*Config::lobaRobustHuberWidth());
                optimizer.addEdge(e);
                ...
                std::shared_ptr<Feature> ftr_frame2 = pt->findFrameRef(frame2);
                ...
                double& final_error)
        {
            ...
            list<EdgeContainerSE3> edges;
            set<std::shared_ptr<Point>> mps;
            list<FramePtr> neib_kfs;
            size_t v_id = 0;
            size_t n_mps = 0;
            size_t n_fix_kfs = 0;
            size_t n_var_kfs = 1;
            ...
            {
                std::shared_ptr<g2oFrameSE2> v_kf = createG2oFrameSE2(it_kf->get(), v_id++, false);
                (*it_kf)->v_kf_ = v_kf;
                ++n_var_kfs;
            ...
            for(set<Point*>::iterator it_pt = mps.begin(); it_pt!=mps.end(); ++it_pt)
            {
                ...
                while(it_obs!=(*it_pt)->obs_.end())
                {
                    ...
                    if((*it_obs)->frame->v_kf_ == NULL)
                    {
                        ...
                        // is fixed. create one:
                        std::shared_ptr<g2oFrameSE2> v_kf = createG2oFrameSE2((*it_obs)->frame, v_id++, true);
                    ...
                    // create edge
                    std::shared_ptr<g2oEdgeSE3> e = createG2oEdgeSE3((*it_obs)->frame->v_kf_, v_pt,
            ...
            for(list<FramePtr>::iterator it = neib_kfs.begin(); it != neib_kfs.end(); ++it)
                (*it)->v_kf_ = NULL;

            // Update Mappoints
            for(set<std::shared_ptr<Point>>::iterator it = mps.begin(); it != mps.end(); ++it)
            {
        ...
        void globalBA(Map* map)
        {
                ...
                it_kf != map->keyframes_.end(); ++it_kf)
            {
                ...
                for(Features::iterator it_ftr=(*it_kf)->fts_.begin(); it_ftr!=(*it_kf)->fts_.end(); ++it_ftr)
                {
                    ...
                    {
                        std::shared_ptr<g2oEdgeSE3> e = createG2oEdgeSE3(v_kf, v_mp, vk::project2d((*it_ftr)->f),
                                                         true,
                                                         reproj_thresh_2*Config::lobaRobustHuberWidth());
  ...
  g2o::OptimizationAlgorithmLevenberg* solver = new g2o::OptimizationAlgorithmLevenberg(solver_ptr);
        ...
        g2oFrameSE3*
        createG2oFrameSE3(FramePtr frame, size_t id, bool fixed)
        {
